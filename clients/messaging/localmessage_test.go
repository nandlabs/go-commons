...
func TestMessage_SetBody(t *testing.T) {
	message := LocalMessage{
		...
		headerTypes: map[string]reflect.Kind{
			"Content-Type": reflect.String,
		},
	}
	_, err := message.SetBodyStr("This is the message body")

	if err != nil {
		t.Errorf("error setting body: %v", err)
	}
	buffer := new(bytes.Buffer)
	_, err = buffer.ReadFrom(message.ReadBody())
	if err != nil {
...
//	// Create a new LocalMessage instance for testing
//	message := LocalMessage{
//		headers: map[string]interface{}{
//			"Content-Type":   "text/plain",
...
//	}
//
//	message.SetBoolHeader("Custom-Header3", true)
//
//	tests := []struct {
//		key          string
...
//		value, exists := message.GetBoolHeader(test.key)
//
